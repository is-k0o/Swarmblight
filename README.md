...
V0.6.5.6A adds `python forge.py fidelity-check-decomposed <card-id> --repeat N` for an existing card (N = 1–5). It is a separate diagnostic reviewer, with no production admission wiring. It loads the card and exact source provenance through the read-only knowledge store, using the existing Forge run/session for budget attribution. Each non-empty gate-owned field gets one call with independent verdicts for every original item index; empty optional fields are covered without calls. Code rejects missing, duplicate, unexpected, or misattributed results. Only after all items pass does one relationship-only cross-field call run. Every call uses `FIDELITY_MAX_OUTPUT_TOKENS` for both authorization and the provider (at most 13 calls per complete review).

The command prints each independent run's coverage, issues, status/verdict, provider response IDs, per-call usage, and totals. Any item or cross-field failure gives FAIL; provider or coverage errors never give PASS. Retryable incomplete/coverage failures restart the next repetition from the beginning; cancellation, budget, accounting and other non-retryable errors stop repetitions. `incomplete_count` includes error runs (also reported separately as `error_count`). Unknown usage is reported as unknown, not zero cost, while existing BudgetManager accounting retains the conservative charge. There is no vote, rewrite, persisted decomposed review, migration, or knowledge-state change. Production `SourceFidelityGate` and its admission decisions remain unchanged.
